...
//    col = baseCol + 4;
  } else if((type == AC_LOSE_EFFECT || type == AC_EFFECT || type == AC_REFRESH) && event == "魔人のレクイエム") {
    col = baseCol + 5;
    ...
    col = baseCol + 8;
  } else if(whom == userName && (type == AC_LOSE_EFFECT || type == AC_AOE　|| type == AC_ACTION || type == AC_REFRESH) && event == "アーゼマの均衡") {
    col = baseCol + 9;
  } else if(whom == userName && (type == AC_LOSE_EFFECT || type == AC_EFFECT) && event == "強化薬") {
    col = baseCol + 10;
  } else if(whom == userName && (type == AC_LOSE_EFFECT || type == AC_EFFECT || type == AC_REFRESH) && event == "リフレッシュ") {
    col = baseCol + 11;
  }
